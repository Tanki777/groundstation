...
class CompassWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Draw satellite
        self.draw_satellite(painter, center, radius * 0.5)
    ...
    def draw_satellite(self, painter, center, body_radius):
        """Draw a simple satellite at the center."""
        painter.save()  # Save the current state of the painter
        painter.translate(center)  # Move to the center
        painter.rotate(self.heading)  # Rotate based on the heading

        # Draw satellite body (circle)
        painter.setPen(Qt.GlobalColor.black)
        painter.setBrush(QColor("gray"))
        painter.drawEllipse(QPointF(0, 0), body_radius, body_radius)

        # Draw left solar panel
        panel_width, panel_height = body_radius * 1.5, body_radius * 0.3
        painter.setBrush(QColor("blue"))
        painter.drawRect(int (-panel_width - body_radius), int (-panel_height / 2), int (panel_width), int (panel_height))

        # Draw right solar panel
        painter.drawRect(int (body_radius), int (-panel_height / 2), int (panel_width), int (panel_height))
